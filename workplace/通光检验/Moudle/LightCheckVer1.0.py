...
class MyWindow(QMainWindow, Ui_MainWindow):
    ...
    def message_waring(self, value):
        receive = QtWidgets.QMessageBox.warning(self, "保存提示", str(value), QtWidgets.QMessageBox.Yes|QtWidgets.QMessageBox.No)
    ...
    def thread_start(self, *args, **kwargs):
        ...
        try:
            ...
            print(e)
        if len(self.lineEdit_branch_SN.text()) > 0:
            flag = True
        core_num = self.textEdit_preimport_branch.toPlainText().count("\n")
        polarity_num = self.textEdit_polarityInput.toPlainText().count("-")
        if core_num == int(self.lineEdit_productionItem.text()) and polarity_num == int(self.lineEdit_productionItem.text()) * int(self.lineEdit_productionCore.text()) or flag:
    ...
    def PreSn_check(self, *args, **kwargs):
        """对于组数和极性进行限制与检测, 并传输分支极性"""
        try:
            core_box = []
            num_core = int(self.lineEdit_productionCore.text())
            num_item = int(self.lineEdit_productionItem.text())
    ...
    def collect_window_new(self,value):
        ...
        for i, n in enumerate(polarity_config):
            ...
            if len(self.lineEdit_branch_SN.text()) > 0:
                branch_sn = f"{self.lineEdit_branch_SN.text()}-{str(b_flag+1).zfill(3)}"
                b_flag += 1
            elif value[i].split("-", 1)[-1] != "0":
...
class Translate(QtCore.QThread):
    ...
    def receive_data(self):
        try:
            ...
            for i in back:
                backWords.append(f"{back_dict[i[1]]}-{back_dict[i[0]]}")
            backWords = backWords[::-1]
            return backWords
        except Exception as e:
